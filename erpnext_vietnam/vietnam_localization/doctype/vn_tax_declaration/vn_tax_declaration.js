...
frappe.ui.form.on("VN Tax Declaration", {
    refresh(frm) {
        if (frm.is_new() || frm.doc.status === "Voided" || frm.doc.adapter_status !== "Ready") {
            return;
        }
        frm.add_custom_button(__("Review XLSX"), () => {
            download_review(frm, "erpnext_vietnam.declarations.service.download_tax_declaration_review", "xlsx");
        }, __("Export"));
        frm.add_custom_button(__("Review XML"), () => {
            download_review(frm, "erpnext_vietnam.declarations.service.download_tax_declaration_review", "xml");
        }, __("Export"));
    },
});
