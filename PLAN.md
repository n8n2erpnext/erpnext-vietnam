...
- [P5A gate CLOSED 2026-09-06] Read-only Vietnam Localization Health reports Company setup, accounting/VAT mapping readiness, payroll/e-invoice readiness, gateway state and production-endpoint certification pins without changing data. ERPNext `Global Defaults` now contains an app-owned Vietnam Localization section with status plus Setup/Health buttons, so operators do not need to remember Desk routes. Live validation on `LightBI Inc` returned `READY_WITH_WARNINGS`, correctly identifying empty COA mappings and missing e-invoice profile while keeping Compliance Gateway disabled.
- [P5B gate CLOSED 2026-09-06] Read-only release doctor verifies supported Frappe/ERPNext v16, optional HRMS v16, critical DocTypes/Pages, 15/15 business-profile seeds, setup-snapshot SHA-256 integrity, sandbox-production exclusion and production certification pins. Live doctor on `erp.thaiduy.digital` returned PASS with 0 failures/0 warnings on Frappe 16.17.0 / ERPNext 16.16.0 / HRMS 16.5.4.
- [P5C gate CLOSED 2026-09-06] A disposable MariaDB site completed clean Frappe -> ERPNext -> `erpnext_vietnam` install, Release Doctor PASS, explicit migrate replay, second Doctor PASS, 15/15 business-profile seeds, zero localization settings/endpoints/submissions before setup, and Global Defaults Vietnam entry fields present. The disposable site/database/user were removed afterward; production remained HTTP 200. SQLite remains excluded from certification because its upstream ERPNext install path is experimental and locked during testing.

## Release candidate
- `0.1.0-rc1` is the first engineering RC after P0-P5 gates. It is not a claim of accountant/legal acceptance and includes no real provider adapter without separately pinned official technical material.
